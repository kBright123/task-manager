...
# ---- 扩展初始化与 SQLite 加固 ----
# ---- 数据模型(抽离至 models.py, 此处再导出保持 from app import X 兼容) ----
...
from kb.knowledge import init_models, enable_sqlite_wal, kb_bp, \
...
def _soft_delete_exclude(orm_exec_state):
    if orm_exec_state.is_select and not orm_exec_state.execution_options.get(
...
from core.app_services import (get_job_setting, set_job_setting, client_ip,
                          ...
                          create_notification, _get_cached_notifications,
                          _set_cached_notifications,
                          _clear_cached_notifications, get_same_group_users,
                          _count_notes, _count_kb, TASK_COMPLETION_DAYS,
