...
# ===== av(PyAV) 启动预装(edge-tts 的 MPEG-2/24k 音频在 iPad 静默失败, 需 av 转码成
#       MPEG-1/44.1k; wheel 自带 ffmpeg 库, 无需系统 ffmpeg) =====
# KB_PREINSTALL=0 关闭; 与 jionlp 共用 KB_AUTOPIP_TIMEOUT / KB_PIP_MIRROR
if [ "${KB_PREINSTALL:-1}" = "1" ]; then
  if python -c "import av" >/dev/null 2>&1; then
    echo "[entrypoint][av] 已安装, 跳过预装"
  else
    PIP_TIMEOUT="${KB_AUTOPIP_TIMEOUT:-600}"
    MIRROR="${KB_PIP_MIRROR-https://pypi.tuna.tsinghua.edu.cn/simple}"
    LOG=/app/instance/av_pip.log
    mkdir -p /app/instance 2>/dev/null || true
    echo "== $(date '+%F %T') pip install av (timeout=${PIP_TIMEOUT}s mirror=${MIRROR:-pypi官方}) ==" > "$LOG" 2>/dev/null || LOG=/tmp/av_pip.log
    echo "[entrypoint][av] 开始预装(超时${PIP_TIMEOUT}s, 源:${MIRROR:-pypi官方})..."
    START=$(date +%s)
    set +e
    if [ -n "$MIRROR" ]; then
      timeout "$PIP_TIMEOUT" python -m pip install --no-cache-dir \
    ...
    set -e
    ELAPSED=$(( $(date +%s) - START ))
    echo "[entrypoint][av] 结束 rc=${RC} 耗时${ELAPSED}s (完整日志: ${LOG})"
    if [ "$RC" -eq 0 ]; then
      echo "[entrypoint][av] 预装成功"
    elif [ "$RC" -eq 124 ]; then
      echo "[entrypoint][av] 判定: 超时=网络过慢/被墙, 日志尾部:" >&2
      tail -n 15 "$LOG" >&2 || true
    else
      echo "[entrypoint][av] 判定: 无法下载(DNS/源不可达/权限), rc=${RC}, 日志尾部:" >&2
      tail -n 25 "$LOG" >&2 || true
    fi
    chown appuser:appuser "$LOG" 2>/dev/null || true
  fi
